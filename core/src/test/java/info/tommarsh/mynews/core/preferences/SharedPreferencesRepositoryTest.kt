...
class SharedPreferencesRepositoryTest {
    ...
    fun `get country`() {
        val expected = "uk"
        whenever(preferences.getString(KEY_PREFERENCE_COUNTRY, any()))
            .thenReturn("uk")
    ...
    fun `get night mode`() {
        ...
        whenever(preferences.getInt(KEY_PREFERENCE_NIGHT_MODE, any())).thenReturn(MODE_NIGHT_NO)
    ...
    fun `get if onboarding should be shown`() {
        val expected = true
        whenever(preferences.getBoolean(KEY_PREFERENCE_SHOW_ONBOARDING, any())).thenReturn(true)
