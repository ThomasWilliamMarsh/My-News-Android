...
class CategoryChoiceAdapter : ListAdapter<CategoryViewModel, CategoryChoiceViewholder>(callback) {

    companion object {
        val callback = getDiffUtilItemCallback<CategoryViewModel> { old, new -> old.id == new.id }
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = CategoryChoiceViewholder(parent)
